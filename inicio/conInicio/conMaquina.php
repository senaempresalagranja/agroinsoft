...
<button type="button" class="button btn-ayuda" data-type="zoomin" value="ayuda"   type="button"><img class="icon-ayuda"  src="../iconos/ayuda.svg" alt="x" /></button> 
	...
	<input class="cam-input" type="text" name="Ubicacion" placeholder="Ubicacion" id="Ubicacion">
	<input class="cam-input" type="text" name="Clasificacion" placeholder="Clasificacion" id="Clasificacion">
</div>
	<div class="botones">
...
<form action="Consultar_Detalles.php" method="POST">
<div id="contenedor_consulta">
	...
	<div id="contenedor"></div>
		...
		$insertar="SELECT * FROM `ayuda` WHERE id_formulario=10 order by ayuda.terAyuda";
...
$("#Ubicacion").keyup(Consulta_Automatica);
$("#Ubicacion").keyup(validar_ubicacion);
$("#Clasificacion").keyup(Consulta_Automatica);
$("#Clasificacion").keyup(validar_clasificacion);
...
if (validar_nombre()==true || validar_codigo()==true || validar_clasificacion()==true || validar_ubicacion()==true ) {	
var nombre_maquina=document.getElementById('nombre_maquina').value;
var codigo_interno=document.getElementById('codigo_interno').value;
var Ubicacion=document.getElementById('Ubicacion').value;
var Clasificacion=document.getElementById('Clasificacion').value;

$("#contenedor_consulta").load("Consulta_Automatica.php",{nombre_maquina:nombre_maquina,codigo_interno:codigo_interno,Ubicacion:Ubicacion,Clasificacion:Clasificacion })
...
var nombre_maquina=document.getElementById('nombre_maquina').style.border="2px solid green"
		...
		function validar_ubicacion() {
			var Ubicacion=document.getElementById('Ubicacion').value; Ubicacion=Ubicacion.toLowerCase();
			if(Ubicacion==null  || Ubicacion.length==0 || /[\\^"'<>;ç`,-_ª%&¿¨Ç¡º#~¬€$.*+?=!:|\\/()\[\]{}]/.test(Ubicacion)){
			var Ubicacion=document.getElementById('Ubicacion').style.border="2px solid red"
				return false;

			}else if (isNaN(Ubicacion)==false) {
				var Ubicacion=document.getElementById('Ubicacion').style.border="2px solid red"
				return false;
...
var Ubicacion=document.getElementById('Ubicacion').style.border="2px solid green"
			...
			var Clasificacion=document.getElementById('Clasificacion').value; Clasificacion=Clasificacion.toLowerCase();
			if(Clasificacion==null  || Clasificacion.length==0 || /[\\^"'<>;ç`,-_ª%&¿¨Ç¡º#~¬€$.*+?=!:|\\/()\[\]{}]/.test(Clasificacion)){
			var Clasificacion=document.getElementById('Clasificacion').style.border="2px solid red"
				return false;

			}else if (isNaN(Clasificacion)==false) {
				var Clasificacion=document.getElementById('Clasificacion').style.border="2px solid red"
				return false;

			}else if (Clasificacion.length>50) {
				var Clasificacion=document.getElementById('Clasificacion').style.border="2px solid red"
				return false;

			}else{

var Clasificacion=document.getElementById('Clasificacion').style.border="2px solid green"
